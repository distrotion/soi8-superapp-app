...
import '../../page/P31QCSAPMASTER/P31QCSAPMASTERvar.dart';
...
class QCSAPMASTER_SET extends QCSAPMASTER_Event {}
...
class QCSAPMASTER_Bloc extends Bloc<QCSAPMASTER_Event, List<QCSAPMASTERitem>> {
  QCSAPMASTER_Bloc() : super([]) {
    ...
    on<QCSAPMASTER_SET>((event, emit) {
      return _QCSAPMASTER_SET([], emit);
    });
  ...
  Future<void> _QCSAPMASTER_SET(
      List<QCSAPMASTERitem> toAdd, Emitter<List<QCSAPMASTERitem>> emit) async {
    List<QCSAPMASTERitem> output = [];
    FreeLoading(P31QCSAPMASTERmaincontext);
    final response = await Dio().post(
      server + "PRODUCTIONHISTORY/SETUPDATE",
      data: {
        "MKMNR": P31QCSAPMASTERvar.MKMNR,
        "ITEM": P31QCSAPMASTERvar.SETTOSAP,
      },
    );
    if (response.statusCode == 200) {
      var databuff = response.data;
      // print(databuff);
      P31QCSAPMASTERmaincontext.read<QCSAPMASTER_Bloc>().add(QCSAPMASTER_GET());
      Navigator.pop(P31QCSAPMASTERmaincontext);
    }
    Navigator.pop(P31QCSAPMASTERmaincontext);
    emit(output);
